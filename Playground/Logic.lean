...
lemma if_true' {c : Prop} [Decidable c] {t : α} {e : α} (h : c) :
  (if c then t else e) = t
  := by
    apply (ite_eq_left_iff (P := c)).mpr
    intro hc
    contradiction
lemma if_false' {c : Prop} [Decidable c] {t : α} {e : α} (h : ¬c) :
  (if c then t else e) = e
  := by
    rw [← ite_not c, if_true' h]

lemma dite_true' {c : Prop} [Decidable c] {t : c → α} {e : ¬c → α} (h : c) :
  ...
  := by
    ...
    intro hc
    contradiction
lemma dite_false' {c : Prop} [Decidable c] {t : c → α} {e : ¬c → α} (h : ¬c) :
